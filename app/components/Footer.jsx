...
const guides = [
    { label: "Mastering Conversational Commerce", href: "/guides/conversational-commerce" },
    ...
    { label: "Ace Black Friday & Cyber Monday Sale", href: "/guides/black-friday" },
    { label: "Mastering Conversational AI", href: "/guides/conversational-ai" },
    { label: "A guide to WhatsApp Business API", href: "/guides/whatsapp-business-api" },
    { label: "Win buyers' Hearts with Conversational AI", href: "/guides/win-buyers" },
];
            ...
            <div className="max-w-7xl mx-auto px-6 py-10">
                <div className="flex flex-col lg:flex-row gap-10 justify-between">
                    ...
                    <div className="flex-shrink-0 max-w-xs">
                        {/* Placeholder Logo */}
                        <Link href="https://www.gupshup.ai/" >
                            <Image src="https://gs-upload.gupshup.io/revamp/assets/v3/images/new-home-page/new-logo-gupshup.svg" alt="Gupshup Logo" width={180} height={180} />
                        </Link>
                        <p className="text-sm text-gray-600 leading-relaxed mt-4">
                            Gupshup is a leading conversational AI platform for marketing, commerce, and support automation. Trusted by 50,000+ customers in 130+ countries across industries, Gupshup handles 120 Billion+ messages annually.
                        </p>
                        <Link
                            href="#"
                    ...
                    <div className="min-w-[140px]">
                        <h3 className="text-sm font-medium text-gray-600 mb-4 pb-2 border-b border-gray-300">Company</h3>
                        <ul className="space-y-3">
                            {companyLinks.map((item) => (
                                <li key={item.label}>
                                    <Link href={item.href} className="text-sm text-gray-500  transition-colors">
                                        {item.label}
                                    </Link>
                                </li>
                            ))}
                    ...
                    <div className="min-w-[280px]">
                        <h3 className="text-sm font-medium text-gray-600 mb-4 pb-2 border-b border-gray-300">Resources</h3>
                        <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                            {resourceLinks.map((item) => (
                                <Link key={item.label} href={item.href} className="text-sm text-gray-500 transition-colors">
                                    {item.label}
                                </Link>
                            ))}
                        </div>
                    ...
                    <div className="min-w-[120px]">
                        <h3 className="text-base font-semibold text-right text-gray-900 mb-4">Follow Us</h3>
            ...
            <div className="border-t border-gray-200 bg-white">
                <div className="max-w-7xl mx-auto px-6 py-4 flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap gap-4 text-xs text-indigo-600">
                        {legalLinks.map((item) => (
                            <Link key={item.label} href={item.href} className="hover:text-indigo-800 transition-colors text-indigo-600">
                                {item.label}
                            </Link>
                        ))}
                        <Link href="#" className="">
                            <span className="text-gray-600 text-sm">Partner Terms of Use</span>
                        </Link>
                    </div>
                    <span className="text-xs text-gray-500">Gupshup © 2026. All rights reserved.</span>
                </div>
            </div>
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-600">
                            {guides.map((guide, i) => (
                                <span key={guide.label} className="flex items-center gap-3">
                                    <Link href={guide.href} className="hover:text-indigo-600 transition-colors">
                                        {guide.label}
                                    </Link>
                                    {i < guides.length - 1 && <span className="text-gray-400">|</span>}
                                </span>
                            ))}
                        </div>
                    </div>
